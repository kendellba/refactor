...
export const useOnboardingStore = defineStore('onboarding', () => {
  ...
  const submitBasicInfoFormData = async (currentBasicInfoData) => {
    ...
    try {
      ...
      const responseData = response.data as BasicInfoApiResponse;
      
      // On success, update the demoStore using the proper method
      if (demoStore.setBasicInfo) {
        // Map API response back to store format
        const storeData = {
          firstName: responseData.first_name,
          lastName: responseData.last_name,
          otherName: responseData.middle_name,
          email: responseData.email,
          mobileNumber: responseData.mobile,
          schoolName: responseData.school_name,
          gender: responseData.gender,
          dob: responseData.dob,
          maritalStatus: responseData.marital_status,
          nationality: responseData.nationality,
        };
        demoStore.setBasicInfo(storeData);
      }
      
      if (responseData.id) {
        demoStore.setSignupId(responseData.id.toString());
      }
  ...
  const submitForeignNationalBankData = async (currentBankData) => {
    ...
    try {
      ...
      if (demoStore.setForeignNationalInfo) {
  ...
  const submitMailingAddressData = async (currentMailingAddressData) => {
    ...
    if (currentMailingAddressData.sameAsResidential) {
      ...
      const residential = (demoStore.addressInfo as any) || {};
      demoStore.$patch({
        mailingAddressInfo: {
  ...
  const verifyMobileCodeAction = async (verificationCode) => {
    ...
    try {
      ...
      demoStore.$patch({
        ...
        mobileVerifiedOn: (response as any).data?.verified_on || new Date().toISOString(),
      });
  ...
  const skipPowerOfAttorneyAction = () => {
    demoStore.$patch({ skipPowerOfAttorney: true });
          ...
          errorData.detail.forEach(err => {
            if (err.loc && err.loc.length > 1) {
              ...
              if (fieldName === 'account_number') {
                (parsedFieldErrors as any).account_number = err.msg;
              }
            }
          });
  ...
  return {
    ...
    sendMobileVerificationCodeAction,
    verifyMobileCodeAction,
    setCustomerAsNew,
    setCustomerAsExisting,
    initializeCustomerType,
    submitMembershipDeclaration,
    submitPepDataAction,
    submitPowerOfAttorneyAction,
    skipPowerOfAttorneyAction,
    verifyAccountNumberAction,
    finalizeOnboarding,
  };
});
