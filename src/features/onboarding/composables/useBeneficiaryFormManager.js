...
export function useBeneficiaryFormManager() {
  ...
  const loadState = () => {
    ...
    } else if (demoStore.beneficiaryInfo && demoStore.beneficiaryInfo.length > 0) {
      addedBeneficiariesList.value = JSON.parse(JSON.stringify(demoStore.beneficiaryInfo)); // Deep copy
    }
  ...
  const clearPersistedStates = () => {
    ...
    // demoStore.beneficiaryInfo might be cleared at a higher level (e.g. end of flow) or be explicitly managed
  };
