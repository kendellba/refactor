...
export function useEmailVerificationFormManager() {
  ...
  const userEmail = computed(() => (demoStore as any).email || sessionStorage.getItem('verifyingEmail') || '');
  ...
  const buildApiParams = (operation, includeCode = false) => {
    ...
    if (isLoginVerification.value) {
      (params as any).email = (demoStore as any).email;
    } else {
      (params as any).signupId = (demoStore as any).signupId;
    }
    
    if (includeCode) {
      (params as any).code = formData.value.verificationCode || '';
    }
  ...
  const getCurrentUserId = () => {
    return isLoginVerification.value ? (demoStore as any).email : (demoStore as any).signupId;
  };
  ...
  const requestNewVerificationCode = async () => {
    ...
    const result = await onboardingStore.requestEmailVerificationCodeAction({
      signupId: currentId,
  ...
  const submitVerificationCode = async () => {
    ...
    try {
      const result = await onboardingStore.verifyEmailCodeAction({
        signupId: currentId,
        email: userEmail.value,
        operation: params.operation,
      ...
      if (result.success) {
        ...
        if (result.fieldMessages?.verificationCode) {
          fieldErrors.value = {
            ...fieldErrors.value,
            verificationCode: result.fieldMessages.verificationCode
          };
        } else if (result.generalMessage?.toLowerCase().includes('code')) {
          fieldErrors.value = {
            ...fieldErrors.value,
            verificationCode: result.generalMessage
          };
        }
      }
  ...
  onMounted(async () => {
    ...
    if (!(demoStore as any).signupId && !isLoginVerification.value) {
      formSubmitError.value = 'Missing registration details. Please start from basic information.';
      return;
    }

    if (isLoginVerification.value && !(demoStore as any).email) {
