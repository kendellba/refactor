...
// Type definition to match schema
...
export function useMembershipDeclarationFormManager() {
  ...
  const formData = ref<MembershipDeclarationFormData>({
    isMemberOfAnotherCreditUnion: 'no' as const, // Explicit type
    creditUnionName: '',
    isServingOnBoard: 'no' as const, // Explicit type
  ...
  } = useSimpleFormValidation(membershipDeclarationSchema);
