...
import type { IdInformation } from '@/types/store';
...
export function useHolderType() {
  ...
  const determineHolderType = (dateOfBirth: string): string => {
  ...
  const getHolderTypeFromStore = (): string => {
  ...
  const updateHolderType = (newHolderType: string): void => {
    console.log('Updating holder type in store to:', newHolderType);
    if (store.idInformation) {
      store.setIdInformation({
        ...store.idInformation,
        holder_type: newHolderType,
      });
    } else {
      // If idInformation doesn't exist yet, create it with default values
      const defaultIdInfo: IdInformation = {
        firstIdType: '',
        firstIdNumber: '',
        firstExpiryDate: '',
        secondIdType: '',
      ...
      store.setIdInformation(defaultIdInfo);
    }
  };
